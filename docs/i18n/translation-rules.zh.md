...
本仓库的 Markdown 约定对 `.zh.md` 文件原样生效：一个段落一个物理行、相对链接必须可解析、文件末尾恰好一个换行。
...
- 请运行 `pnpm run verify-translation-pairing`。门禁会检查一致性记录、切换行、标题深度、代码块、表格行列数、列表类型、有序列表起始编号、列表项数量、链接及仓库 Markdown 规则；列表与表格的顺序、非常规列表编号、行内代码、强调标记、语义、术语和语体则由人工评审负责。
