...
| `@oh-dsh/vision` | 适配 [`dsh-vision`](https://github.com/william-jin-cmu/dsh-vision) | 跨三端的 `view_image` Host 工具，以及 Desktop/Web 输入区的粘贴图片缩略图 Client；复用 DSH credentials、settings、attachment 与输入触发器 |
...
- Desktop/Web 粘贴图片先进入 DSH attachment store，再通过随机、不透明且绑定 Session
  的引用交给 `view_image`；引用记录位于统一数据根目录，不能跨 Session 解析。
